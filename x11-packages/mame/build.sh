...
termux_step_pre_configure() {
	# sed -i 's|ext_lib("lua")|ext_lib("lua++")|' scripts/src/main.lua scripts/src/3rdparty.lua
	sed -i '1i #include <sys/prctl.h>' 3rdparty/bx/src/thread.cpp
}

termux_step_make() {
	local target_cflags="$CFLAGS -U__ANDROID__"
	local target_cxxflags="$CXXFLAGS -U__ANDROID__ "
...
termux_step_make_install() {
	...
	install -Dm644 -t "${TERMUX_PREFIX}/share/applications" "${TERMUX_PKG_BUILDER_DIR}/mame.desktop"
	install -Dm644 -t "${TERMUX_PREFIX}/share/icons/hicolor/scalable/apps" "${TERMUX_PKG_BUILDER_DIR}/mame.svg"

	mkdir -p "$TERMUX_PREFIX/share/mame"
	cp -ar artwork bgfx plugins language ctrlr keymaps hash "$TERMUX_PREFIX/share/mame/"

	{
		mkdir -p "$TERMUX_PREFIX/share/mame/shader"
		cp -a src/osd/modules/opengl/shader/glsl*.*h "$TERMUX_PREFIX/share/mame/shader/"
		# Fix keycode names for UP, DOWN, LEFT, RIGHT.
		# sed -i -E 's|KEYCODE_([2468])_PAD|KEYCODE_\1PAD|g' "$TERMUX_PREFIX/share/mame/ctrlr"/*.cfg
	} 2>/dev/null || true
	...
	install -Dm644 docs/LICENSE "$TERMUX_PREFIX/share/licenses/mame/LICENSE"
}
