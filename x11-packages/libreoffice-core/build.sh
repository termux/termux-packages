...
TERMUX_PKG_DESCRIPTION="LibreOffice core files shared by all components"
...
TERMUX_PKG_SHA256=42116e256933aa575974e420ffa04f7cd7096f4b7ca5d0907ddeaf2a07f68f94
TERMUX_PKG_AUTO_UPDATE=true
# Files of the former monolithic "libreoffice" package moved here.
TERMUX_PKG_BREAKS="libreoffice (<< 26.8.0.3-1)"
TERMUX_PKG_REPLACES="libreoffice (<< 26.8.0.3-1)"
...
termux_step_post_make_install() {
	local list_dir="$TERMUX_PKG_TMPDIR/file-lists"
	local destdir="$TERMUX_PKG_TMPDIR/list-destdir"
	local gid_dir="$list_dir/gid"

	rm -rf "$list_dir" "$destdir"
	mkdir -p "$gid_dir" "$destdir"
	make -j 1 install DESTDIR="$destdir" ||
		{ rm -rf "$destdir"; termux_error_exit "Installing into a temporary DESTDIR failed"; }
	cp "$destdir"/gid_Module_* "$gid_dir/" ||
		{ rm -rf "$destdir"; termux_error_exit "The installer did not write gid_Module_* lists to $destdir"; }
	rm -rf "$destdir"
	[[ -f "$gid_dir/gid_Module_Root" ]] ||
		termux_error_exit "gid_Module_Root is missing, can not split LibreOffice into components"

	local -A modules=(
		[base]="gid_Module_Prg_Base_Bin gid_Module_Brand_Prg_Base gid_Module_Reportbuilder"
		[calc]="gid_Module_Prg_Calc_Bin gid_Module_Brand_Prg_Calc"
		[draw]="gid_Module_Prg_Draw_Bin gid_Module_Brand_Prg_Draw gid_Module_Pdfimport"
		[impress]="gid_Module_Prg_Impress_Bin gid_Module_Brand_Prg_Impress gid_Module_Optional_OGLTrans"
		[math]="gid_Module_Prg_Math_Bin gid_Module_Brand_Prg_Math"
		[writer]="gid_Module_Prg_Wrt_Bin gid_Module_Brand_Prg_Wrt"
	)

	# Lists hold absolute paths; "%dir" lines are directories only and are skipped.
	# Directories are expanded so that the lists contain files and symlinks only.
	local app module path
	for app in "${!modules[@]}"; do
		: > "$list_dir/$app.files"
		for module in ${modules[$app]}; do
			[[ -f "$gid_dir/$module" ]] || continue
			sed -n "s|^${TERMUX_PREFIX}/||p" "$gid_dir/$module" |
				while IFS= read -r path; do
					if [[ -d "$TERMUX_PREFIX/$path" && ! -L "$TERMUX_PREFIX/$path" ]]; then
						(cd "$TERMUX_PREFIX" && find "$path" \( -type f -o -type l \))
					elif [[ -e "$TERMUX_PREFIX/$path" || -L "$TERMUX_PREFIX/$path" ]]; then
						echo "$path"
					fi
				done >> "$list_dir/$app.files"
		done
		sort -u -o "$list_dir/$app.files" "$list_dir/$app.files"
	done

	# A file listed by several components (e.g. libsdlo.so for Draw and Impress)
	# must stay in libreoffice-core, or only the first subpackage would get it.
	cat "$list_dir"/*.files | sort | uniq -d > "$list_dir/shared"

	for app in "${!modules[@]}"; do
		grep -vxFf "$list_dir/shared" "$list_dir/$app.files" > "$list_dir/$app.unique" || true
		# The subpackage include loop splits on whitespace and expands globs,
		# so any unusual character (e.g. spaces in template names) becomes "?".
		sed 's|[^A-Za-z0-9_./+@%,=:~-]|?|g' "$list_dir/$app.unique" > "$list_dir/$app.include"
		[[ -s "$list_dir/$app.include" ]] ||
			termux_error_exit "No files found for libreoffice-$app, check the gid_Module_* lists in $gid_dir"
		echo "INFO: libreoffice-$app: $(wc -l < "$list_dir/$app.include") files"
	done
}

# The package is tracked on Repology as "libreoffice" (the metapackage), not "libreoffice-core".
termux_pkg_auto_update() {
	local latest_version
	latest_version="$(termux_repology_api_get_latest_version libreoffice)"
	if [[ "${latest_version}" == "null" || "${latest_version}" == "${TERMUX_PKG_VERSION}" ]]; then
		echo "INFO: Already up to date."
		return 0
	fi

	local candidate_srcurl="https://download.documentfoundation.org/libreoffice/src/${latest_version%.*}/libreoffice-${latest_version}.tar.xz"
	if ! curl --fail --silent --head --location --output /dev/null "${candidate_srcurl}"; then
		echo "INFO: Repology reports libreoffice ${latest_version}, but ${candidate_srcurl} isn't published yet. Skipping update for now."
		return 0
	fi
