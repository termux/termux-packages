...
termux_step_pre_configure() {
	...
	if [[ "$TERMUX_ON_DEVICE_BUILD" == "false" ]]; then
		TERMUX_PKG_EXTRA_MAKE_ARGS+=" --qmake=$TERMUX_PREFIX/lib/qt6/bin/host-qmake6"
	else
		TERMUX_PKG_EXTRA_MAKE_ARGS+=" --qmake=$TERMUX_PREFIX/bin/qmake6"
	fi
...
termux_step_make() {
	...
	if [[ "$TERMUX_ON_DEVICE_BUILD" == "true" ]]; then
		_sip_build="sip-build"
	else
		_sip_build="python ${TERMUX_PYTHON_CROSSENV_PREFIX}/build/bin/sip-build"
	fi
	${_sip_build} \
