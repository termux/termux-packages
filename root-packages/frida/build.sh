...
termux_step_pre_configure() {
	...
	if [ $TERMUX_ARCH == "aarch64" ]; then
		TERMUX_PKG_EXTRA_CONFIGURE_ARGS+=" --host=android-arm64"
	elif [ $TERMUX_ARCH == "i686" ]; then
		TERMUX_PKG_EXTRA_CONFIGURE_ARGS+=" --host=android-x86"
	else
		TERMUX_PKG_EXTRA_CONFIGURE_ARGS+=" --host=android-$TERMUX_ARCH"
	fi
}
