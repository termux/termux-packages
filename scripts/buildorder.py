...
class TermuxPackage(object):
    ...
    def __init__(self, dir_path, fast_build_mode):
        ...
        pkgdir = os.path.dirname(self.virtual_src) if self.virtual else self.dir
        for filename in os.listdir(pkgdir):
            if not filename.endswith('.subpackage.sh'):
                continue
            subpkg = TermuxSubPackage(pkgdir + '/' + filename, self)
            if termux_arch in subpkg.excluded_arches:
                continue
            self.subpkgs.append(subpkg)

        subpkg = TermuxSubPackage(pkgdir + '/' + self.name + '-static' + '.subpackage.sh', self, virtual=True)
        self.subpkgs.append(subpkg)
    ...
    def recursive_dependencies(self, pkgs_map, pkg_root=None, only_installing=False):
        ...
        if is_root or not self.fast_build_mode or not self.separate_subdeps:
            ...
            if not self.fast_build_mode or self.dir == pkg_root.dir:
                self.deps.difference_update([subpkg.name for subpkg in self.subpkgs])
        for dependency_name in sorted((self.pkg_deps if only_installing else self.deps)-self.virtual_antideps):
            ...
            if dependency_name not in self.pkgs_cache:
                ...
                dep_only_installing = (dependency_package.dir != pkg_root.dir and dependency_package.only_installing and not self.fast_build_mode)
                if dep_only_installing or self.fast_build_mode:
                    result += dependency_package.recursive_dependencies(pkgs_map, pkg_root, dep_only_installing)
                if not dep_only_installing and (dependency_package.accept_dep_scr or dependency_package.dir != pkg_root.dir) and not(pkg_root.virtual and os.path.basename(dependency_package.dir) == os.path.basename(pkg_root.dir)):
