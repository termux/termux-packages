termux_setup_toolchain_gnu() {
	...
	if [ ! -d "$TERMUX_PREFIX_LIB/" ]; then
		termux_error_exit "glibc library directory was not found ('$TERMUX_PREFIX_LIB/')"
	fi
	if [ ! -f "$TERMUX_PREFIX_LIB/libgcc_s.so" ] && [ ! -f "$TERMUX_PREFIX_LIB/libgcc_s.so.1" ]; then
	...
	export PATH_DYNAMIC_LINKER="$TERMUX_PREFIX_BASE_LIB/$DYNAMIC_LINKER"
