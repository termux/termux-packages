termux_step_create_debian_package() {
	...
	{ # add control fields to package (note: at time of writing, $TERMUX_PKG_PRE_DEPENDS is not used for anything)
	[[ "$TERMUX_PKG_ESSENTIAL" == "true" ]] && echo "Essential: yes"
	[[ -n "$TERMUX_PKG_DEPENDS"          ]] && echo "Depends: ${TERMUX_PKG_DEPENDS/#, /}"
	[[ -n "$TERMUX_PKG_PRE_DEPENDS"      ]] && echo "Pre-Depends: ${TERMUX_PKG_PRE_DEPENDS/#, /}"
	[[ -n "$TERMUX_PKG_BREAKS"           ]] && echo "Breaks: $TERMUX_PKG_BREAKS"
	[[ -n "$TERMUX_PKG_CONFLICTS"        ]] && echo "Conflicts: $TERMUX_PKG_CONFLICTS"
	[[ -n "$TERMUX_PKG_RECOMMENDS"       ]] && echo "Recommends: $TERMUX_PKG_RECOMMENDS"
	[[ -n "$TERMUX_PKG_REPLACES"         ]] && echo "Replaces: $TERMUX_PKG_REPLACES"
	[[ -n "$TERMUX_PKG_PROVIDES"         ]] && echo "Provides: $TERMUX_PKG_PROVIDES"
	[[ -n "$TERMUX_PKG_SUGGESTS"         ]] && echo "Suggests: $TERMUX_PKG_SUGGESTS"
	echo "Description: $TERMUX_PKG_DESCRIPTION"
	} >> control

	# Create DEBIAN/conffiles (see https://www.debian.org/doc/debian-policy/ap-pkg-conffiles.html):
	for f in $TERMUX_PKG_CONFFILES; do echo "$TERMUX_PREFIX_CLASSICAL/$f" >> conffiles; done
