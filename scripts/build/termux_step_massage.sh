termux_step_massage() {
	...
	# Configure pkgconfig files for proper multi-compilation
	if [[ -d ./${ADDING_PREFIX}/lib32/pkgconfig ]]; then
		...
		for pc in $(grep -s -r -l "^includedir=.*/include32" ./${ADDING_PREFIX}/lib32/pkgconfig); do
			...
			local pc_multi_path="$(grep '^includedir=' "${pc}" | sed "s|${TERMUX_PREFIX}|\${prefix}|g" | awk -F '{prefix}/include' '{printf $2}')"
			local pc_edit_cflags="$(sed "s|\${includedir}|\${includedir}${pc_multi_path}|g" <<< "${pc_cflags}")"
			...
			sed -i -e "s|\(^includedir=.*/\)include32\(.*\)|\1include|g" \
				...
				"${pc}"
			# Apply the modified pkgconfig to the system for proper multi-compilation work
