...
async function getAptPackages(repo, arch, errors, _proposedAutomatedFixes, proposedManualFixes) {
      ...
      // New line indicates the end of a package entry
        ...
        if (pkgName && pkgVersion) {
          if (aptPackages.has(pkgName)) {
            ...
            try {
              await execFileAsync('dpkg', ['--compare-versions', pkgVersion, 'ge', aptPackages.get(pkgName).version]);
...
async function getTermuxPackages(arch, errors, _proposedAutomatedFixes, proposedManualFixes) {
  const termuxPackages = new Map();
  // "./scripts/apt-packages-list-${arch}.txt" is a file generated by
  // `./scripts/generate-apt-packages-list.sh` script, which is run
...
async function getErrorsForArch(arch) {
  ...
  const termuxPackages = await getTermuxPackages(arch, errors, proposedAutomatedFixes, proposedManualFixes);
  const aptPackages = new Map();
  for (const path in repos) {
