...
termux_pkg_upgrade_version() {
	...
	local space_available="$(df "/var/lib/docker" | awk 'NR==2 { val=$4*1024 } END { if(!val) val='$cleanup_threshold'+1; print val }')"
	...
	if [[ "${big_package}" == "true" ]] || (( space_available <= cleanup_threshold )); then
		"${TERMUX_SCRIPTDIR}/scripts/run-docker.sh" ./clean.sh
	fi
