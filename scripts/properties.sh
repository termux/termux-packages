...
##
# Set the path to the `termux-packages` repo root directory.
##
__termux_build_props__set_termux_builder__repo_root_dir() {
    TERMUX_BUILDER__REPO_ROOT_DIR=""
    return_value=0
    if [[ "$(readlink --help 2>&1 || true)" =~ [\ ]-d[,\ ] ]]; then
        TERMUX_BUILDER__REPO_ROOT_DIR="$(file="$(readlink -f -- "${BASH_SOURCE[0]}")" && parent="$(dirname -- "$file")" && readlink -f -- "$parent/..")" || return_value=$?
    else
        TERMUX_BUILDER__REPO_ROOT_DIR="$(pwd)" || return_value=$? # macOS `< 12.3` compatibility.
    fi
    if [ $return_value -ne 0 ] || [[ -z "$TERMUX_BUILDER__REPO_ROOT_DIR" ]] || [[ ! -f "$TERMUX_BUILDER__REPO_ROOT_DIR/build-package.sh" ]]; then
        echo "The TERMUX_BUILDER__REPO_ROOT_DIR '$TERMUX_BUILDER__REPO_ROOT_DIR' not found or is not valid." 1>&2
        return 1;
    fi
}

if [[ -z "${TERMUX_BUILDER__REPO_ROOT_DIR:-}" ]]; then
...
if [[ "${TERMUX_PACKAGES_OFFLINE-false}" == "true" ]]; then
    export ANDROID_HOME="${TERMUX_BUILDER__REPO_ROOT_DIR}/build-tools/android-sdk-${TERMUX_SDK_REVISION}"
    export NDK="${TERMUX_BUILDER__REPO_ROOT_DIR}/build-tools/android-ndk-r${TERMUX_NDK_VERSION}"
else
...
##
TERMUX_PACKAGES__REPO_NAME="termux-packages"
...
##
TERMUX_PACKAGES__REPO_URL="$TERMUX__REPOS_HOST_ORG_URL/$TERMUX_PACKAGES__REPO_NAME"
...
TERMUX_PACKAGES_DIRECTORIES=$(jq --raw-output 'del(.pkg_format) | keys | .[]' "$TERMUX_BUILDER__REPO_ROOT_DIR/repo.json")

for url in $(jq -r 'del(.pkg_format) | .[] | .url' "$TERMUX_BUILDER__REPO_ROOT_DIR/repo.json"); do
    TERMUX_REPO_URL+=("$url")
done
for distribution in $(jq -r 'del(.pkg_format) | .[] | .distribution' "$TERMUX_BUILDER__REPO_ROOT_DIR/repo.json"); do
    TERMUX_REPO_DISTRIBUTION+=("$distribution")
done
for component in $(jq -r 'del(.pkg_format) | .[] | .component' "$TERMUX_BUILDER__REPO_ROOT_DIR/repo.json"); do
