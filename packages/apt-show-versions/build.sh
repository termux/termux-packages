...
termux_step_pre_configure() {
	...
	local -a _MAKE_INSTALL_PROVIDED_FILES=(
		"bin/apt-show-versions"
		"share/man/man1/apt-show-versions.1p"
	)

	local marker_file
	for marker_file in "${_MAKE_INSTALL_PROVIDED_FILES[@]}"; do
		rm -f "$TERMUX_PREFIX/$marker_file"
	done
}
...
termux_step_post_make_install() {
	install -Dm644 "$TERMUX_PKG_SRCDIR/debian/apt-show-versions.apt.conf" \
		"$TERMUX_PREFIX/etc/apt/apt.conf.d/20apt-show-versions"

	install -Dm644 "$TERMUX_PKG_SRCDIR/debian/apt-show-versions.bash-completion" \
