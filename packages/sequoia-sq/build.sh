...
termux_step_host_build() {
	...
	read -r CAPNPROTO_VERSION CAPNPROTO_SHA256 < <(
		. "$TERMUX_SCRIPTDIR/packages/capnproto/build.sh"
		echo "$TERMUX_PKG_VERSION $TERMUX_PKG_SHA256"
	)
	...
	LDFLAGS="-Wl,-rpath=${TERMUX_PKG_HOSTBUILD_DIR}/.libs" ./configure
	make -j"${TERMUX_PKG_MAKE_PROCESSES}"
}

termux_step_pre_configure() {
	PATH+=":${TERMUX_PKG_HOSTBUILD_DIR}/.libs"
	termux_setup_rust
