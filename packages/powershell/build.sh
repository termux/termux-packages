...
termux_step_pre_configure() {
	...
	# Disable AppHost across all projects to prevent host-pack issues on Android and cross-compilation host binary contamination
	cat <<- PROPS > Directory.Build.props
	<Project>
	  <PropertyGroup>
...
termux_step_make_install() {
	...
	rm -fv "${TERMUX_PKG_BUILDDIR}/publish/pwsh"
	...
	export LD_LIBRARY_PATH="${TERMUX_PREFIX}/lib/powershell:${TERMUX_PREFIX}/lib:\${LD_LIBRARY_PATH}"
	exec dotnet "${TERMUX_PREFIX}/lib/powershell/pwsh.dll" "\$@"
	EOL
