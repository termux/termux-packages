...
termux_step_make() {
 perl Makefile.PL EXPATLIBPATH=$TERMUX_PREFIX/lib EXPATINCPATH=$TERMUX_PREFIX/include PREFIX=$TERMUX_PREFIX AR=$AR CC=$CC RANLIB=$RANLIB CFLAGS=$CFLAGS LDFLAGS=$LDFLAGS CCFLAGS+= "$CPPFLAGS"
  make -j 4
 }
