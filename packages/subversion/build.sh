...
TERMUX_PKG_MAKE_INSTALL_TARGET="install install-swig-pl"

termux_step_post_extract_package() {
	(cd subversion/bindings/swig/
	cp proxy/swig_perl_external_runtime.swg perl/libsvn_swig_perl/)
}

termux_step_pre_configure() {
	CFLAGS+=" -std=c11"
	CPPFLAGS+=" -I$TERMUX_PREFIX/include/perl"
}
