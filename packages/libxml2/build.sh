...
termux_step_post_make_install() {
	# the `xmlcatalog.1` and `xmllint.1` man pages end up in the wrong dir for some reason.
	# So move them manually to where they belong.
	mv "$TERMUX_PREFIX/share/man/"*.1 "$TERMUX_PREFIX/share/man/man1"
}
