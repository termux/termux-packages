...
TERMUX_PKG_SRCURL=https://mirrors.kernel.org/gnu/bash/bash-${_MAIN_VERSION}.${_PATCH_VERSION}.tar.gz
...
termux_step_pre_configure() {
	...
	for PATCH_NUM in $(seq -f '%03g' ${_PATCH_VERSION}); do
		...
		termux_download \
			"https://mirrors.kernel.org/gnu/bash/bash-${_MAIN_VERSION}-patches/bash${_MAIN_VERSION/./}-0${_PATCH_VERSION}" \
			$PATCHFILE \
