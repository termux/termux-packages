...
termux_step_make() {
	...
	$CC -shared -fPIC $LDFLAGS -o libunibilium.so unibilium.o uninames.o uniutil.o
}

termux_step_make_install() {
	install -m600 libunibilium.so $TERMUX_PKG_MASSAGEDIR/$TERMUX_PREFIX/lib/
	install -m600 unibilium.h $TERMUX_PKG_MASSAGEDIR/$TERMUX_PREFIX/include/
	...
	sed "s|@VERSION@|$TERMUX_PKG_VERSION|" unibilium.pc.in | \
		...
		$TERMUX_PKG_MASSAGEDIR/$PKG_CONFIG_LIBDIR/unibilium.pc
}
