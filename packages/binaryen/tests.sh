...
[ $([ "${TERMUX_APP_PACKAGE_MANAGER-}" = "apt" ] && echo "debian" || echo "${TERMUX_APP_PACKAGE_MANAGER-}") == "pacman" ] && (
     ...
     pacman -Syy cmake libxml2 libxslt ndk-sysroot openjdk-22 python-lxml python-numpy python-ensurepip binaryen --noconfirm
     ...
     apt --fix-missing --fix-broken install cmake libxml2 libxslt ndk-sysroot openjdk-22 python-lxml python-numpy python-ensurepip binaryen --yes
