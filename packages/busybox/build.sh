...
termux_step_make_install() {
	...
	install -Dm700 ./0_lib/busybox_unstripped $TERMUX_PKG_MASSAGEDIR/$TERMUX_PREFIX/bin/busybox
	install -Dm700 ./0_lib/libbusybox.so.${TERMUX_PKG_VERSION}_unstripped $TERMUX_PKG_MASSAGEDIR/$TERMUX_PREFIX/lib/libbusybox.so.${TERMUX_PKG_VERSION}
	ln -sfr $TERMUX_PREFIX/lib/libbusybox.so.${TERMUX_PKG_VERSION} $TERMUX_PKG_MASSAGEDIR/$TERMUX_PREFIX/lib/libbusybox.so
