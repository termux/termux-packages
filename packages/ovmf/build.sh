...
termux_step_post_get_source() {
	cp -f $TERMUX_PKG_BUILDER_DIR/License.txt $TERMUX_PKG_CACHEDIR/
	cd $TERMUX_PKG_CACHEDIR
	bsdtar -xf "edk2.git-aarch64-0-${TERMUX_PKG_VERSION}.noarch.rpm" -C $TERMUX_PREFIX
	bsdtar -xf edk2.git-ovmf-x64-0-${TERMUX_PKG_VERSION}.noarch.rpm -C $TERMUX_PREFIX
}
