...
termux_step_make() {
	...
	sed -i 's/int namelen/size_t namelen/g' libschily/getdomainname.c
	sed -i 's/int[[:space:]]*namelen;/size_t namelen;/g' libschily/getdomainname.c
	sed -i 's/int namelen/size_t namelen/g' include/schily/hostname.h
	sed -i '1i #define HAVE_SETREUID 1' bsh/bsh.c
	sed -i '360i #ifndef HAVE_SHMID_DS\nstruct shmid_ds { int dummy; };\n#endif' cdda2wav/semshm.c
	sed -i '1i #define NO_SHADOW\n#undef HAVE_GETSPNAM\n#undef HAVE_GETSPWNAM\n#undef HAVE_SHADOW_H' bsh/idops.c
	sed -i 's/#if defined(HAVE_GETSPNAM) || defined(HAVE_GETSPWNAM)/#if 0 \/* Android Fix *\//g' bsh/idops.c
	sed -i 's/.*error  No function.*/\/* UID controlled by external SU *\//' bsh/idops.c
	sed -i '/to set uid available/d' bsh/idops.c
	sed -i '/#include "bsh.h"/a struct spwd { char *sp_pwdp; };' bsh/idops.c
	export CFLAGS="$CFLAGS -D_GNU_SOURCE -DNO_SHADOW"
	./psmake/smake \
		ARCH="$TERMUX_ARCH" \
		OSNAME=linux \
		CCOM=clang \
