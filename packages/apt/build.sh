...
TERMUX_PKG_SHA256=89a0757a076b30adfad5772be3d7eab66659f69639b120200942933d154b55ee
# apt-key requires utilities from coreutils, findutils, gpgv, grep, sed.
TERMUX_PKG_DEPENDS="coreutils, dpkg, findutils, gpgv, grep, libandroid-glob, libbz2, libc++, libgcrypt, libgnutls, libiconv, liblz4, liblzma, libseccomp, sed, termux-keyring, termux-licenses, xxhash, zlib, zstd"
...
termux_step_create_debscripts() {
	...
	[ -f "$TERMUX_PREFIX/etc/apt/sources.list" ] && {
		echo
		echo "================================"
		echo "Legacy repo file found for 'main' repository."
		echo "The legacy 'sources.list' entry will be migrated"
		echo "to 'sources.list.d/main.sources' as part of the update."
		cp -vaf \\
			"$TERMUX_PREFIX"/etc/apt/sources.list \\
			"$TERMUX_PREFIX"/etc/apt/sources.list.dpkg-old
		echo "A backup copy of the old sources.list repo file"
		echo "has been saved to $TERMUX_PREFIX/etc/apt/sources.list.dpkg-old"
		echo "================================"
		echo
	}
	exit 0
	...
	[ -f "$TERMUX_PREFIX/etc/apt/sources.list" ] && rm "$TERMUX_PREFIX/etc/apt/sources.list"
	EOF
