...
# pinky has no usage on Android. realpath has permission denied problem with relative paths,
# so let the busybox realpath remain in PATH:
TERMUX_PKG_EXTRA_CONFIGURE_ARGS+="  --enable-no-install-program=pinky,realpath"
